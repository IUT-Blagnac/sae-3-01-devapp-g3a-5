...
|{ts} |{us_}16"] |{is_t_o}50 "] {is_c_c}51 "] {is_d_o}52 "]|{doc_t}protocole-de-communication "] {doc_u}voir-données-equipes"]|{tst}aaa"]|Connection au port serie
|{ts} |{us_}3"] |{is_t_o}50 "] {is_c_c}51 "] {is_d_o}52 "]|{doc_t}protocole-de-communication "] {doc_u}voir-données-equipes"]|{tst}aaa"]|Couleur aleatoire des joueurs 
