...
|{ts} |{us_}66"] |{is_t_o}67 "] {is_c_c}68 "] {is_d_o}69 "]|{doc_t}protocole-de-communication "] {doc_u}"]|{tst}aaa"]|Accès Port Série
|{ts} |{us_}62"] |{is_t_o}63 "] {is_c_c}64 "] {is_d_o}65 "]|{doc_t}données-admin"] {doc_u}"]|{tst}aaa"]|Données Admin
