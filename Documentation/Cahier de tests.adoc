...
:test_wt: image:{badge}Test-WIP-orange.svg[]
...
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Connecter un sniffeur avant de se rendre sur la page index. ^|Le sniffeur se lance sans problème, et apparait sur la page. ^| Être sur la page index, disposer d'un sniffeur. ^|{test_wt}
5+|
