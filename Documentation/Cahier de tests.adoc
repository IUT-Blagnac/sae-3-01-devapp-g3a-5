...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Trouver un checkpoint avec Sniffeur 1. ^|Le Sniffeur 1 se déplace sur la case suivante. ^| Sniffeur 1 est connecté. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Trouver un checkpoint avec Sniffeur 1. ^|Seul le Sniffeur 1 se déplace sur la case suivante. ^| Sniffeur 1 et Sniffeur 2 sont connectés. ^|{test_wt}
...
// Définissez les informations de votre test!
:num_test: 1
:nom_test: Téléchargement Rapport JSON
:objectif: S'assurer que le rapport JSON se télécharge.
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Appuyer sur le bouton de téléchargement du JSON. ^|Le téléchargement est lancé. ^| Être sur la page admin, avoir une partie lancée et avancée. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Appuyer sur le bouton de téléchargement du JSON. ^|Le JSON est téléchargé. ^| Être sur la page admin, avoir une partie lancée et avancée. Ne pas être connecté à Internet. ^|{test_wt}
5+| Commentaire : Aucun commentaire.
|====
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Lire manuellement les données d'un JSON. ^|Les données sont conformes aux besoins du client. ^| Avoir un JSON ouvert sur un outil de développement. ^|{test_wt}
5+| Commentaire : Aucun commentaire.
|====
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Trouver un checkpoint avec Sniffeur 1. ^|L'IHM indique sur le tableau que l'équipe correspondant au Sniffeur 1 a trouvé le premier checkpoint. ^| Sniffeur 1 est connecté. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Trouver un checkpoint avec Sniffeur 1. ^|Seul l'équipe de Sniffeur 1 est mise à jour sur le tableau. ^| Sniffeur 1 et Sniffeur 2 sont connectés. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.3* ^|Trouver un checkpoint avec Sniffeur 2. ^|L'IHM indique sur le tableau que l'équipe correspondant au Sniffeur 1 a trouvé le premier checkpoint. ^| Avoir effectué le test {lvl1}.{lvl2}.*{num_test}.2*. ^|{test_wt}
