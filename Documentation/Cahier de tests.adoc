...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Appuyer sur le bouton pause. ^|Le mode pause s'active. ^| Être sur la page admin, ne pas être en pause. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Appuyer sur le bouton pause. ^|Le mode pause s'arrête. ^| Être sur la page admin, être en pause. ^|{test_wt}
5+|
|====
...
// Définissez les informations de votre test!
:num_test: 2
:nom_test: Fonctionnalités Pause
:objectif: S'assurer que les fonctionnalités du mode Pause sont effectives
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Appuyer sur le bouton pause 2 fois, avec 10 secondes d'écart. ^|Cela ne dérègle pas le résultat des temps. ^| Être sur la page admin, ne pas être en pause. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Matraquer le bouton pause. ^|Cela ne dérègle pas le résultat des temps. ^| Être sur la page admin, ne pas être en pause. ^|{test_wt}
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Envoyer un message aux spectateurices. ^|Le message s'envoie. ^| Être sur la page admin. ^|{test_wt}
5+|
|====
...
^|{lvl1}.{lvl2}.*{num_test}.1* ^|Envoyer un message "bonjour", puis se rendre sur la page des spectateurs. ^|Le message est visible sur la page des spectateurs. ^| Être sur la page admin. ^|{test_wt}
^|{lvl1}.{lvl2}.*{num_test}.2* ^|Envoyer un message "Voix ambiguë d’un cœur qui, au zéphyr, préfère les jattes de kiwis.", puis se rendre sur la page des spectateurs. ^|Le message est visible sur la page des spectateurs, et ne comporte pas d'erreur UTF-8. ^| Être sur la page admin. ^|{test_wt}
...
* "Rafraichissement Index" 
* "Rapport de partie"
* "Cohérence des temps des sniffeurs"
* "Cohérence des couleurs de joueurs"
* "Fonctionnement de la coche 'trouvé' "
* "Avancement des pions"
* "Intégrité de taille du tableau"
